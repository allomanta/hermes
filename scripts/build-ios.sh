...
[ -n "${HERMES_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${HERMES_ORIG_GROUP}.app/group.${HERMES_NEW_GROUP}.app/g" "ios/Hermes Share/Hermes Share.entitlements"
	sed -i "" "s/group.${HERMES_ORIG_GROUP}.app/group.${HERMES_NEW_GROUP}.app/g" "ios/Runner/Runner.entitlements"
	sed -i "" "s/group.${HERMES_ORIG_GROUP}.app/group.${HERMES_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
	# Bundle identifiers
	sed -i "" "s/${HERMES_ORIG_GROUP}.app/${HERMES_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
}

[ -n "${HERMES_NEW_TEAM}" ] && {
	# Code signing team
	sed -i "" "s/${HERMES_ORIG_TEAM}/${HERMES_NEW_TEAM}/g" "ios/Runner.xcodeproj/project.pbxproj"
}
cat << EOHELP
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for hermes.app and hermes.app.Hermes-Share)
...
[ -n "${HERMES_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/hermes.ipa"
  rm -rf "${TMPDIR}"
}
